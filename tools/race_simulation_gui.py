...
class RaceSimulationApp:
    ...
    def _build_ui(self) -> None:
        ...
        help_text = (
            "操作说明：\n"
            "1. 左键点击画布空白处可新增一个光电门。\n"
            "2. 左键点中光电门后，可围绕门中心拖动鼠标旋转到任意角度。\n"
            "3. 双击光电门可删除。\n"
            "4. 绿色为下一个目标门，蓝色为已通过门，红色为当前目标门。\n"
            "5. 决赛预设会显示 10 个引导门，便于按规则模拟整场比赛。"
        )
    ...
    def on_canvas_click(self, event: tk.Event[tk.Canvas]) -> None:
        ...
        if gate_index is not None:
            ...
            self.gate_rotating = True
            self.status_var.set(f"选中第 {self.gates[gate_index].gate_id} 个光电门，拖动可旋转")
            self._update_selected_gate_text()
            self._rotate_gate_to_cursor(event.x, event.y)
            self._draw_scene()
            return
    ...
    def _rotate_gate_to_cursor(self, canvas_x: int, canvas_y: int) -> None:
        ...
        angle_deg = self._wrap_angle(math.degrees(math.atan2(world_y - gate.y, world_x - gate.x)))
        self.gates[self.selected_gate_index] = Gate(
            gate_id=gate.gate_id,
            x=gate.x,
            y=gate.y,
