...
static int rampTowards(int currentValue, int targetValue, int maxStep) {
...
// 红外导航控制，带强制停止选项
void motor_control_ir_navigation(int directionErrorDeg, bool forceStop) {
    ...
    directionErrorDeg = constrain(directionErrorDeg, -180, 180);
    int absError = abs(directionErrorDeg);

    int baseSpeed = 130 - (absError * 45) / 180;
    if (absError < 25) {
        baseSpeed = 135;
    } else if (absError > 150) {
        baseSpeed = 75;
    }
    baseSpeed = constrain(baseSpeed, 70, 135);

    int turn = (directionErrorDeg * 150) / 180;
    int leftTarget = baseSpeed - turn;
    int rightTarget = baseSpeed + turn;

    leftTarget = constrain(leftTarget, -150, 150);
    rightTarget = constrain(rightTarget, -150, 150);

    currentLeft = rampTowards(currentLeft, leftTarget, 12);
    currentRight = rampTowards(currentRight, rightTarget, 12);

    motor_control(0, currentLeft);
    motor_control(1, currentRight);
}